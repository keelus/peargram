...
export function RenderDateShort(UnixDateInt) {
    if (!UnixDateInt)
        return "-1y";
    ...
    if (DiffSeconds < SEC.ONE_HOUR)
        return `${Math.floor(DiffSeconds / SEC.ONE_MINUTE)}m`;
    if (DiffSeconds < SEC.ONE_DAY)
        return `${Math.floor(DiffSeconds / SEC.ONE_HOUR)}h`;
    if (DiffSeconds < SEC.ONE_WEEK)
        return `${Math.floor(DiffSeconds / SEC.ONE_DAY)}d`;
    if (DiffSeconds < SEC.ONE_MONTH)
        return `${Math.floor(DiffSeconds / SEC.ONE_WEEK)}w`;
    if (DiffSeconds < SEC.ONE_YEAR)
        return `${Math.floor(DiffSeconds / SEC.ONE_MONTH)}m`;
